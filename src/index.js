...
function localTime(date, { utc = false } = {}) {
  ...
  return {
    ...
    time24: date.toLocaleString('default', { hour: 'numeric', hour12: false, minute: '2-digit', timeZone }),
    time12: date
...
function displayLocalTime(date, { utc = false, showYear = true } = {}) {
  ...
  const { weekday, monthShort, day, hour, time24, time12, year } = parts;

  const displayTime = hour <= 12 ? time12 : time24; // 10 am
